...
export const testBasics = () => {
  ...
  t.assert(dt.$transformer(delta.$delta({ attrs: { a: s.$string, b: s.$string } }), delta.$delta({ attrs: { b: s.$string } })).validate(i1))
  const $d3 = delta.$delta({ children: 42, attrs: { a: s.$string } })
  const r31 = dt.pipe(r3, r1)
  const i3 = r3.init($d3)
  t.assert(dt.$transformer($d3, delta.$delta({ attrs: { a: s.$string } })).validate(i3))
  const i31 = r31.init($d3)
  t.assert(dt.$transformer($d3, delta.$delta({ attrs: { b: s.$string } })).validate(i31))
  ...
  t.assert(dt.$transformer($da, delta.$delta({ attrs: { b: s.$string } })).validate(p12init))

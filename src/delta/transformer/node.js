 ...
 */
export class AsTransformer extends Transformer {
  /**
   * @param {string} name
   */
  constructor (name) {
...
/**
 * Template for {@link AsTransformer}.
 */
export class As extends Template {
  /**
   * @param {string} name
   */
  constructor (name) {
   ...
   */
  init (_$d) {
    return new AsTransformer(this.target)
  }
}

/**
 * Create an {@link As} template that relabels a node to `name` (side B) and back (side A).
 *
 * @param {string} name
 */
export const as = name => new As(name)

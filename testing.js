...
/* istanbul ignore next */
const testFilterRegExp =
  testFilter !== null ? new RegExp(testFilter) : new RegExp('.*')
...
export const run = async (moduleName, name, f, i, numberOfTests) => {
  ...
  const timeInfo =
    repeat && err === null
      ? ` - ${times.length} repetitions in ${time.humanizeDuration(
          duration
        )} (best: ${time.humanizeDuration(
          times[0]
        )}, worst: ${time.humanizeDuration(
          array.last(times)
        )}, median: ${time.humanizeDuration(
          statistics.median(times)
        )}, average: ${time.humanizeDuration(statistics.average(times))})`
...
const compareValues = (constructor, a, b, path) => {
  if (a !== b) {
    fail(
      `Values ${json.stringify(a)} and ${json.stringify(
        b
      )} don't match (${path})`
    )
  }
 ...
 * @throws {TestError}
 */
export const assert = (condition, message = null) =>
  condition || fail(`Assertion failed${message !== null ? `: ${message}` : ''}`)
...
export const runTests = async tests => {
  const numberOfTests = object
    .map(tests, mod =>
      object
        .map(mod, f => (/* istanbul ignore next */ f ? 1 : 0))
        .reduce(math.add, 0)
    )
    .reduce(math.add, 0)

/*
import { Tree as RedBlackTree } from 'funlibTree.js'
...
test(`random tests (${numberOfRBTreeTests})`, async function randomRBTree (t) {
  ...
  for (var i = 0; i < numberOfRBTreeTests; i++) {
    if (random.int31(prng, 0, 100) < 80) {
      // 80% chance to insert an element
      let obj = ID.createID(random.int31(prng, 0, numberOfRBTreeTests), random.int31(prng, 0, 1))
